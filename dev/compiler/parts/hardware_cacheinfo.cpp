  ...
  while (value != 0)
    {
      ...
      if (byte == 0x40)
      ...
      else if (byte == 0xff)
        {
	      ...
	      //typedef rt enum { null = 0, data = 1, inst = 2, uni = 3 } rt;
	      int mtype = eax & 0x1f;
              if (mtype == 0)
                /* That was the end.  */
                break;
      ...
      else
        {
          ...
          search.idx = byte;
          found = (intel_02_cache_info *)bsearch (&search, intel_02_known, nintel_02_known,
...
handle_intel (int name)
{
  const struct cpu_features *cpu_features = get_cpu_features ();
...
init_cacheinfo (void)
{
  ...
  if (cpu_features->basic.kind == arch_kind_intel)
    {
      data = handle_intel (_SC_LEVEL1_DCACHE_SIZE);
      ...
      /* Try L3 first.  */
      level  = 3;
      shared = handle_intel (_SC_LEVEL3_CACHE_SIZE);
                  ...
                  while (count)
                    {
                      ...
                      int mtype = ecx & 0xff00;
                      if (shipped == 0 || mtype == 0)
                        break;
                      else if (mtype == 0x100)
                        {
                          /* Count SMT.  */
