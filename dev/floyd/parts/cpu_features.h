...
long int __cache_sysconf (int name);
...
/* Used from outside of glibc to get access to the CPU features
   structure.  */
extern const struct cpu_features *get_cpu_features (void)
     __attribute__ ((const));
...
#define HAS_CPU_FEATURE(name) \
  CPU_FEATURES_CPU_P (get_cpu_features (), name)
/* HAS_ARCH_FEATURE evaluates to true if we may use the feature at
   runtime.  */
# define HAS_ARCH_FEATURE(name) \
  CPU_FEATURES_ARCH_P (get_cpu_features (), name)
