...
bc_value_t bc_value_t::make_typeid_value(const itype_t& type_id){
	return bc_value_t{ type_id };
}
itype_t bc_value_t::get_typeid_value() const {
	QUARK_ASSERT(check_invariant());

	return _pod._external->_typeid_value;
}
bc_value_t::bc_value_t(const itype_t& type_id) :
...
bc_value_t bc_value_t::make_struct_value(const itype_t& struct_type, const std::vector<bc_value_t>& values){
...
bc_value_t::bc_value_t(const itype_t& struct_type, const std::vector<bc_value_t>& values, bool struct_tag) :
	_type(struct_type)
{
...
bc_value_t bc_value_t::make_function_value(const itype_t& function_type, const function_id_t& function_id){
...
bc_value_t::bc_value_t(const itype_t& function_type, const function_id_t& function_id, bool dummy) :
	_type(function_type)
{
...
bc_value_t::bc_value_t(const itype_t& type, const bc_pod_value_t& internals) :
	_type(type),
	_pod(internals)
{
...
bc_value_t::bc_value_t(const itype_t& type, const bc_inplace_value_t& pod64) :
	_type(type),
	_pod{._inplace = pod64}
{
	QUARK_ASSERT(type.check_invariant());
...
bc_value_t::bc_value_t(const itype_t& type, const bc_external_handle_t& handle) :
	_type(type),
...
bc_value_t::bc_value_t(const itype_t& type, mode mode) :
	_type(type)
{
	QUARK_ASSERT(type.check_invariant());
...
bool encode_as_vector_w_inplace_elements(const type_interner_t& interner, const itype_t& type){
	...
	return type.is_vector() && encode_as_inplace(type.get_vector_element_type(interner));
}
...
bool encode_as_dict_w_inplace_values(const type_interner_t& interner, const itype_t& type){
	...
	return type.is_dict() && encode_as_inplace(type.get_dict_value_type(interner));
}
...
//				QUARK_ASSERT(_string);
		...
		QUARK_ASSERT(_typeid_value == itype_t::make_undefined());
...
bc_external_value_t::bc_external_value_t(const std::string& s) :
	_rc(1),
#if DEBUG
	_debug_type(itype_t::make_string()),
#endif
	_string(s)
{
	QUARK_ASSERT(check_invariant());
...
bc_external_value_t::bc_external_value_t(const itype_t& type, const function_id_t& function_id) :
	_rc(1),
#if DEBUG
	_debug_type(type),
#endif
...
bc_external_value_t::bc_external_value_t(const itype_t& s) :
	_rc(1),
#if DEBUG
	_debug_type(itype_t::make_typeid()),
#endif
	_typeid_value(s)
{
	QUARK_ASSERT(s.check_invariant());
...
bc_external_value_t::bc_external_value_t(const itype_t& type, const std::vector<bc_value_t>& s, bool struct_tag) :
		_rc(1),
#if DEBUG
	_debug_type(type),
#endif
...
bc_external_value_t::bc_external_value_t(const itype_t& type, const immer::vector<bc_external_handle_t>& s) :
	_rc(1),
#if DEBUG
	_debug_type(type),
#endif
...
bc_external_value_t::bc_external_value_t(const itype_t& type, const immer::vector<bc_inplace_value_t>& s) :
	_rc(1),
#if DEBUG
	_debug_type(type),
#endif
...
bc_external_value_t::bc_external_value_t(const itype_t& type, const immer::map<std::string, bc_external_handle_t>& s) :
	_rc(1),
#if DEBUG
	_debug_type(type),
#endif
...
bc_external_value_t::bc_external_value_t(const itype_t& type, const immer::map<std::string, bc_inplace_value_t>& s) :
	_rc(1),
#if DEBUG
	_debug_type(type),
#endif
...
bool check_external_deep(const type_interner_t& interner, const itype_t& type, const bc_external_value_t* ext){
	...
	if(basetype == base_type::k_struct){
		...
		const auto& element_type  = type.get_dict_value_type(interner);
		if(encode_as_external(element_type)){
			for(const auto& e: ext->_vector_w_external_elements){
				QUARK_ASSERT(e.check_invariant());
			}
...
const immer::vector<bc_value_t> get_vector(const type_interner_t& interner, const bc_value_t& value){
	...
	const auto element_type = value._type.get_vector_element_type(interner);

	if(encode_as_vector_w_inplace_elements(interner, value._type)){
...
const immer::vector<bc_external_handle_t>* get_vector_external_elements(const type_interner_t& interner, const bc_value_t& value){
	...
	QUARK_ASSERT(value.check_invariant());
	QUARK_ASSERT(value._type.is_vector());
	QUARK_ASSERT(encode_as_vector_w_inplace_elements(interner, value._type) == false);
...
const immer::vector<bc_inplace_value_t>* get_vector_inplace_elements(const type_interner_t& interner, const bc_value_t& value){
	...
	QUARK_ASSERT(value.check_invariant());
	QUARK_ASSERT(value._type.is_vector());
	QUARK_ASSERT(encode_as_vector_w_inplace_elements(interner, value._type) == true);
...
const immer::map<std::string, bc_external_handle_t>& get_dict_external_values(const type_interner_t& interner, const bc_value_t& value){
	QUARK_ASSERT(value.check_invariant());
	QUARK_ASSERT(encode_as_dict_w_inplace_values(interner, value._type) == false);
...
const immer::map<std::string, bc_inplace_value_t>& get_dict_inplace_values(const type_interner_t& interner, const bc_value_t& value){
	QUARK_ASSERT(value.check_invariant());
	QUARK_ASSERT(encode_as_dict_w_inplace_values(interner, value._type) == true);
...
bc_value_t update_struct_member_shallow(interpreter_t& vm, const bc_value_t& obj, const std::string& member_name, const bc_value_t& new_value){
	...
	const auto& struct_def = obj._type.get_struct(vm._imm->_program._types);
...
bc_value_t update_vector_element(interpreter_t& vm, const bc_value_t vec, int64_t lookup_index, const bc_value_t& value){
	...
	QUARK_ASSERT(value.check_invariant());
	QUARK_ASSERT(vec._type.get_vector_element_type(vm._imm->_program._types) == value._type);

//	QUARK_TRACE(json_to_pretty_string(interpreter_to_json(vm)));

	const auto element_type = vec._type.get_vector_element_type(vm._imm->_program._types);
	if(encode_as_vector_w_inplace_elements(vm._imm->_program._types, vec._type)){
...
//	QUARK_TRACE(json_to_pretty_string(interpreter_to_json(vm)));
	...
	if(obj1._type.is_string()){
		...
		const auto element_type = obj1._type.get_vector_element_type(vm._imm->_program._types);
		if(lookup_key._type.is_int() == false){
...
static int bc_compare_struct_true_deep(const type_interner_t& interner, const std::vector<bc_value_t>& left, const std::vector<bc_value_t>& right, const itype_t& type){
	QUARK_ASSERT(interner.check_invariant());
	QUARK_ASSERT(type.check_invariant());

	const auto& struct_def = type.get_struct(interner);

	for(int i = 0 ; i < struct_def._members.size() ; i++){
		const auto& member_type = struct_def._members[i]._type;
		int diff = bc_compare_value_true_deep(interner, left[i], right[i], member_type);
		if(diff != 0){
			return diff;
		}
	}
...
static int bc_compare_vectors_obj(const type_interner_t& interner, const immer::vector<bc_external_handle_t>& left, const immer::vector<bc_external_handle_t>& right, const itype_t& type){
	QUARK_ASSERT(interner.check_invariant());
	QUARK_ASSERT(type.check_invariant());
...
static int bc_compare_dicts_obj(const type_interner_t& interner, const immer::map<std::string, bc_external_handle_t>& left, const immer::map<std::string, bc_external_handle_t>& right, const itype_t& type){
	QUARK_ASSERT(interner.check_invariant());
	QUARK_ASSERT(type.check_invariant());
...
int bc_compare_value_exts(const type_interner_t& interner, const bc_external_handle_t& left, const bc_external_handle_t& right, const itype_t& type){
	...
	return bc_compare_value_true_deep(interner, bc_value_t(type, left), bc_value_t(type, right), type);
}

int bc_compare_value_true_deep(const type_interner_t& interner, const bc_value_t& left, const bc_value_t& right, const itype_t& type0){
	QUARK_ASSERT(interner.check_invariant());
	...
	if(type.is_undefined()){
		...
		return bc_compare_struct_true_deep(interner, left.get_struct_value(), right.get_struct_value(), type0);
	}
	else if(type.is_vector()){
		if(false){
		}
		else if(type.get_vector_element_type(interner).is_bool()){
		...
		else if(type.get_vector_element_type(interner).is_int()){
		...
		else if(type.get_vector_element_type(interner).is_double()){
			...
			const auto& left_vec = get_vector_external_elements(interner, left);
			const auto& right_vec = get_vector_external_elements(interner, right);
			return bc_compare_vectors_obj(interner, *left_vec, *right_vec, type0);
		}
	}
	else if(type.is_dict()){
		if(false){
		}
		else if(type.get_dict_value_type(interner).is_bool()){
			return bc_compare_dicts_bool(left._pod._external->_dict_w_inplace_values, right._pod._external->_dict_w_inplace_values);
		}
		else if(type.get_dict_value_type(interner).is_int()){
			return bc_compare_dicts_int(left._pod._external->_dict_w_inplace_values, right._pod._external->_dict_w_inplace_values);
		}
		else if(type.get_dict_value_type(interner).is_double()){
			return bc_compare_dicts_double(left._pod._external->_dict_w_inplace_values, right._pod._external->_dict_w_inplace_values);
		}
		else  {
			QUARK_ASSERT(encode_as_dict_w_inplace_values(interner, type) == false);

			const auto& left2 = get_dict_external_values(interner, left);
			const auto& right2 = get_dict_external_values(interner, right);
			return bc_compare_dicts_obj(interner, left2, right2, type0);
		}
	}
...
bc_static_frame_t::bc_static_frame_t(const type_interner_t& interner, const std::vector<bc_instruction_t>& instrs2, const std::vector<std::pair<std::string, bc_symbol_t>>& symbols, const std::vector<itype_t>& args) :
...
bc_function_definition_t::bc_function_definition_t(
	...
	_function_id(function_id),
	_dyn_arg_count(-1),
	_return_is_ext(encode_as_external(_function_type.get_function_return(interner)))
{
	const auto args2 = function_type.get_function_args(interner); 
...
json_t interpreter_stack_t::stack_to_json(const type_interner_t& interner) const{
	QUARK_ASSERT(interner.check_invariant());
...
bc_value_t call_function_bc(interpreter_t& vm, const bc_value_t& f, const bc_value_t args[], int arg_count){
	...
	//	QUARK_ASSERT(args.size() == host_function._function_type.get_function_args().size());
		...
		if(lookup_full_basetype(vm, result.first) != base_type::k_void){
...
json_t bcvalue_to_json(const type_interner_t& interner, const bc_value_t& v){
	...
	if(v._type.is_undefined()){
		...
		const auto& struct_def = v._type.get_struct(interner);
		for(int i = 0 ; i < struct_def._members.size() ; i++){
...
//			const auto& type = member._type;
			...
			const auto& value2 = bcvalue_to_json(interner, value);
			obj2[key] = value2;
		}
		return json_t::make_object(obj2);
	}
	else if(v._type.is_vector()){
		const auto element_type = v._type.get_vector_element_type(interner);
		...
		if(element_type.is_bool()){
			...
			const auto vec = get_vector_external_elements(interner, v);
			for(int i = 0 ; i < vec->size() ; i++){
				const auto element_value2 = vec->operator[](i);
				result.push_back(bcvalue_to_json(interner, bc_value_t(element_type, element_value2)));
			}
		}
		return result;
	}
	else if(v._type.is_dict()){
		const auto value_type = v._type.get_dict_value_type(interner);
		if(encode_as_dict_w_inplace_values(interner, v._type)){
			const auto entries = get_dict_inplace_values(interner, v);
			std::map<std::string, json_t> result;
			for(const auto& e: entries){
				const auto value2 = e.second;
				result[e.first] = bcvalue_to_json(interner, bc_value_t(value_type, value2));
			}
			return result;
		}
		else{
			const auto entries = get_dict_external_values(interner, v);
			std::map<std::string, json_t> result;
			for(const auto& e: entries){
				const auto value2 = e.second;
				result[e.first] = bcvalue_to_json(interner, bc_value_t(value_type, value2));
...
static json_t bcvalue_and_type_to_json(const type_interner_t& interner, const bc_value_t& v){
	QUARK_ASSERT(interner.check_invariant());
...
static void execute_new_1(interpreter_t& vm, int16_t dest_reg, int16_t target_itype, int16_t source_itype){
	...
	const auto source_itype2 = lookup_itype_from_index(vm._imm->_program._types, source_itype);
	...
	const bc_value_t result = [&]{
		...
		//	Automatically transform a json::string => string at runtime?
			...
			const auto arg = bcvalue_to_json(vm._imm->_program._types, input_value);
...
void execute_new_vector_obj(interpreter_t& vm, int16_t dest_reg, int16_t target_itype, int16_t arg_count){
	...
	QUARK_ASSERT(encode_as_vector_w_inplace_elements(vm._imm->_program._types, target_type) == false);

	const auto& element_type = target_type.get_vector_element_type(vm._imm->_program._types);
...
//	bool is_element_ext = encode_as_external(element_type);
	...
	const auto result = make_vector(vm._imm->_program._types, element_type, elements2);
...
void execute_new_dict_obj(interpreter_t& vm, int16_t dest_reg, int16_t target_itype, int16_t arg_count){
	...
	const auto& element_type = target_type.get_dict_value_type(vm._imm->_program._types);
	QUARK_ASSERT(target_type.is_undefined() == false);
	QUARK_ASSERT(element_type.is_undefined() == false);

	const auto string_type = itype_t::make_string();
	...
	const auto result = make_dict(vm._imm->_program._types, element_type, elements2);
	vm._stack.write_register__external_value(dest_reg, result);
}
...
static void call_native(interpreter_t& vm, const bc_instruction_t& i, const itype_t& function_type){
	QUARK_ASSERT(vm.check_invariant());
...
//	QUARK_TRACE_SS("STACK:  " << json_to_pretty_string(stack.stack_to_json()));
	...
	while(true){
...
#if DEBUG
			stack._debug_types.push_back(itype_t::make_int());
			stack._debug_types.push_back(itype_t::make_void());
#endif
			QUARK_ASSERT(vm.check_invariant());
			break;
		}
		...
		case bc_opcode::k_lookup_element_vector_w_external_elements: {
			QUARK_ASSERT(vm.check_invariant());
			QUARK_ASSERT(stack.check_reg__external_value(i._a));
			QUARK_ASSERT(stack.check_reg_vector_w_external_elements(vm._imm->_program._types, i._b));
			QUARK_ASSERT(stack.check_reg_int(i._c));
		...
		case bc_opcode::k_lookup_element_vector_w_inplace_elements: {
			QUARK_ASSERT(vm.check_invariant());
			QUARK_ASSERT(stack.check_reg_int(i._a));
			QUARK_ASSERT(stack.check_reg_vector_w_inplace_elements(vm._imm->_program._types, i._b));
			QUARK_ASSERT(stack.check_reg_int(i._c));
		...
		case bc_opcode::k_lookup_element_dict_w_external_values: {
			QUARK_ASSERT(stack.check_reg__external_value(i._a));
			QUARK_ASSERT(stack.check_reg_dict_w_external_values(vm._imm->_program._types, i._b));
			QUARK_ASSERT(stack.check_reg_string(i._c));
		...
		case bc_opcode::k_lookup_element_dict_w_inplace_values: {
			QUARK_ASSERT(stack.check_reg_any(i._a));
			QUARK_ASSERT(stack.check_reg_dict_w_inplace_values(vm._imm->_program._types, i._b));
			QUARK_ASSERT(stack.check_reg_string(i._c));
		...
		case bc_opcode::k_pushback_vector_w_external_elements: {
			QUARK_ASSERT(vm.check_invariant());
			QUARK_ASSERT(stack.check_reg_vector_w_external_elements(vm._imm->_program._types, i._a));
			QUARK_ASSERT(stack.check_reg_vector_w_external_elements(vm._imm->_program._types, i._b));
			QUARK_ASSERT(stack.check_reg__external_value(i._c));

			const auto& type = frame_ptr->_symbols[i._a].second._value_type;
			const auto& element_type = type.get_vector_element_type(vm._imm->_program._types);
			...
			const auto vec2 = make_vector(vm._imm->_program._types, element_type, elements2);
		...
		case bc_opcode::k_pushback_vector_w_inplace_elements: {
			QUARK_ASSERT(vm.check_invariant());
			QUARK_ASSERT(stack.check_reg_vector_w_inplace_elements(vm._imm->_program._types, i._a));
			QUARK_ASSERT(stack.check_reg_vector_w_inplace_elements(vm._imm->_program._types, i._b));
			...
			const auto vec = make_vector(vm._imm->_program._types, element_type, elements2);
		...
		case bc_opcode::k_new_vector_w_external_elements: {
			...
			QUARK_ASSERT(encode_as_vector_w_inplace_elements(vm._imm->_program._types, vector_type) == false);
		...
		case bc_opcode::k_comparison_smaller_or_equal: {
			...
			long diff = bc_compare_value_true_deep(vm._imm->_program._types, left, right, type);
		...
		case bc_opcode::k_concat_vectors_w_external_elements: {
			QUARK_ASSERT(stack.check_reg_vector_w_external_elements(vm._imm->_program._types, i._a));
			QUARK_ASSERT(stack.check_reg_vector_w_external_elements(vm._imm->_program._types, i._b));
			QUARK_ASSERT(stack.check_reg_vector_w_external_elements(vm._imm->_program._types, i._c));

			const auto& vector_type = frame_ptr->_symbols[i._a].second._value_type;
			const auto& element_type = vector_type.get_vector_element_type(vm._imm->_program._types);
			QUARK_ASSERT(encode_as_vector_w_inplace_elements(vm._imm->_program._types, vector_type) == false);
			...
			const auto& value2 = make_vector(vm._imm->_program._types, element_type, elements2);
		...
		case bc_opcode::k_concat_vectors_w_inplace_elements: {
			QUARK_ASSERT(stack.check_reg_vector_w_inplace_elements(vm._imm->_program._types, i._a));
			QUARK_ASSERT(stack.check_reg_vector_w_inplace_elements(vm._imm->_program._types, i._b));
			QUARK_ASSERT(stack.check_reg_vector_w_inplace_elements(vm._imm->_program._types, i._c));

			const auto& vector_type = frame_ptr->_symbols[i._a].second._value_type;
			const auto& element_type = vector_type.get_vector_element_type(vm._imm->_program._types);
			QUARK_ASSERT(encode_as_vector_w_inplace_elements(vm._imm->_program._types, vector_type) == true);
...
static std::vector<json_t> bc_symbols_to_json(const type_interner_t& interner, const std::vector<std::pair<std::string, bc_symbol_t>>& symbols){
	QUARK_ASSERT(interner.check_invariant());
	...
	for(const auto& e: symbols){
		...
		if(symbol._const_value._type.is_undefined() == false){
			const auto e2 = json_t::make_array({
				...
				e.first,
				"CONST",
				bcvalue_and_type_to_json(interner, symbol._const_value)
			});
			r.push_back(e2);
		}
		else{
...
static json_t frame_to_json(const type_interner_t& interner, const bc_static_frame_t& frame){
	QUARK_ASSERT(interner.check_invariant());
	...
	return json_t::make_object({
		{ "symbols", json_t::make_array(bc_symbols_to_json(interner, frame._symbols)) },
...
json_t functiondef_to_json(const type_interner_t& interner, const bc_function_definition_t& def){
	QUARK_ASSERT(interner.check_invariant());
