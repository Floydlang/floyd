...
json_t function_def_to_ast_json(const types_t& interner, const function_definition_t& v) {
	...
	auto result = std::vector<json_t>{
		type_to_json(interner, function_type),
		v._definition_name,
...
function_definition_t json_to_function_def(types_t& interner, const json_t& p){
	...
	const type_t function_type1 = type_from_json(interner, function_type0);
...
void trace_function_definition_t(const types_t& interner, const function_definition_t& def){
	QUARK_TRACE_SS(
		...
		<< "\t" << "function_type: " << type_to_compact_string(interner, def._function_type)
...
json_t expression_to_json(const types_t& interner, const expression_t& e){
	struct visitor_t {
		...
		json_t operator()(const expression_t::literal_exp_t& e) const{
			return make_ast_node(
				...
				expression_opcode_t::k_literal,
				{
					...
					type_to_json(interner, e.value.get_type())
				}
			);
		}
		...
		json_t operator()(const expression_t::value_constructor_t& e) const{
			return make_ast_node(
				...
				{
					type_to_json(interner, e.value_type),
	...
	if(e.is_annotated_shallow() && e.has_builtin_type() == false){
		...
		const auto type_json = type_to_json(interner, t);
...
expression_t ast_json_to_expression(types_t& interner, const json_t& e){
	...
	if(op == expression_opcode_t::k_literal){
		...
		const auto type2 = type_from_json(interner, type);
		...
		const auto function_type = type_from_json(interner, e.get_array_n(1));
		...
		const auto value_type = type_from_json(interner, e.get_array_n(1));
