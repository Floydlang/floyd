...
static json_t expressions_to_json(const types_t& types, const std::vector<expression_t> v);
...
static std::string members_to_string(const types_t& types, const std::vector<member_t>& m){
	std::string result;
	for(const auto& e: m){
		const std::string s = std::string("(") + type_to_compact_string(types, e._type) + " " + e._name + std::string(")");
		result = result + s;
	}
...
expression_t::expression_t(const expression_variant_t& contents, const type_t& output_type) :
#if DEBUG_DEEP
	_debug(""),
#endif
...
json_t expression_to_json(const types_t& types, const expression_t& e){
	struct visitor_t {
		...
		json_t operator()(const expression_t::arithmetic_t& e) const{
			return make_ast_node(
				...
				{
					expression_to_json(types, *e.lhs),
					expression_to_json(types, *e.rhs)
				}
			);
		}
		...
		json_t operator()(const expression_t::unary_minus_t& e) const{
			return make_ast_node(floyd::k_no_location, expression_opcode_t::k_unary_minus, { expression_to_json(types, *e.expr) } );
		}
		json_t operator()(const expression_t::conditional_t& e) const{
			const auto a = make_ast_node(
				...
				{
					expression_to_json(types, *e.condition),
					expression_to_json(types, *e.a),
					expression_to_json(types, *e.b)
				}
			);
			return a;
		}
		...
		json_t operator()(const expression_t::call_t& e) const{
			...
			for(const auto& m: e.args){
				args.push_back(expression_to_json(types, m));
			}
			return make_ast_node(floyd::k_no_location, expression_opcode_t::k_call, { expression_to_json(types, *e.callee), json_t::make_array(args) } );
		}
		...
		json_t operator()(const expression_t::struct_definition_expr_t& e) const{
			return make_ast_node(
				...
				{ e.name, json_t::make_array( { members_to_json(types, e.def->_members) } ) }
			);
		}
		json_t operator()(const expression_t::function_definition_expr_t& e) const{
			return make_ast_node(
				...
				function_def_to_ast_json(types, e.def).get_array()
			);
		}
		...
		json_t operator()(const expression_t::resolve_member_t& e) const{
			return make_ast_node(
				...
				{ expression_to_json(types, *e.parent_address), json_t(e.member_name) }
			);
		}
		json_t operator()(const expression_t::update_member_t& e) const{
			return make_ast_node(
				...
				{
					expression_to_json(types, *e.parent_address),
					json_t(e.member_index),
					expression_to_json(types, *e.new_value)
				}
			);
		}
		json_t operator()(const expression_t::lookup_t& e) const{
			return make_ast_node(
				...
				{
					expression_to_json(types, *e.parent_address),
					expression_to_json(types, *e.lookup_key)
				}
			);
		}
		json_t operator()(const expression_t::value_constructor_t& e) const{
			return make_ast_node(
				...
				{
					type_to_json(types, e.value_type),
					expressions_to_json(types, e.elements)
				}
			);
		}
	...
	const json_t result0 = std::visit(visitor_t{ e, types }, e._expression_variant);
	...
	if(e.is_annotated_shallow() && e.has_builtin_type() == false){
		...
		const auto type_json = type_to_json(types, t);
...
static json_t expressions_to_json(const types_t& types, const std::vector<expression_t> v){
	std::vector<json_t> r;
	for(const auto& e: v){
		r.push_back(expression_to_json(types, e));
	}
	return json_t::make_array(r);
}


static type_t get_optional_typeid(types_t& types, const json_t& json_array, int optional_index){
	if(optional_index < json_array.get_array_size()){
		const auto e = json_array.get_array_n(optional_index);
		const auto t = type_from_json(types, e);
		return t;
	}
	else{
...
expression_t ast_json_to_expression(types_t& types, const json_t& e){
	...
	if(op == expression_opcode_t::k_literal){
		...
		const auto expr = ast_json_to_expression(types, e.get_array_n(1));
		const auto annotated_type = get_optional_typeid(types, e, 2);
		...
		const auto lhs_expr = ast_json_to_expression(types, e.get_array_n(1));
		const auto rhs_expr = ast_json_to_expression(types, e.get_array_n(2));
		const auto annotated_type = get_optional_typeid(types, e, 3);
		...
		const auto condition_expr = ast_json_to_expression(types, e.get_array_n(1));
		const auto a_expr = ast_json_to_expression(types, e.get_array_n(2));
		const auto b_expr = ast_json_to_expression(types, e.get_array_n(3));
		const auto annotated_type = get_optional_typeid(types, e, 4);
		...
		const auto function_expr = ast_json_to_expression(types, e.get_array_n(1));
		const auto args = e.get_array_n(2);
		std::vector<expression_t> args2;
		for(const auto& arg: args.get_array()){
			args2.push_back(ast_json_to_expression(types, arg));
		}
		...
		const auto base_expr = ast_json_to_expression(types, e.get_array_n(1));
		...
		const auto new_value_expr = ast_json_to_expression(types, e.get_array_n(3));
		const auto annotated_type = get_optional_typeid(types, e, 4);
		...
		const auto parent_address_expr = ast_json_to_expression(types, e.get_array_n(1));
		const auto lookup_key_expr = ast_json_to_expression(types, e.get_array_n(2));
		const auto annotated_type = get_optional_typeid(types, e, 3);
		...
		const auto members = members_from_json(types, e.get_array_n(2));
		auto def = std::make_shared<struct_type_desc_t>(members);
		return expression_t::make_struct_definition(types, struct_name, def);
	}
		...
		const auto value_type = type_from_json(types, e.get_array_n(1));
		const auto args = e.get_array_n(2).get_array();

		std::vector<expression_t> args2;
		for(const auto& m: args){
			args2.push_back(ast_json_to_expression(types, m));
		}
...
std::string expression_to_json_string(const types_t& types,const expression_t& e){
	const auto json = expression_to_json(types, e);
