...
const llvm_function_link_entry_t& find_function_def_from_link_name(const std::vector<llvm_function_link_entry_t>& function_link_map, const module_symbol_t& link_name){
	auto it = std::find_if(function_link_map.begin(), function_link_map.end(), [&] (const llvm_function_link_entry_t& e) { return e.module_symbol == link_name; } );
...
static std::vector<llvm_function_link_entry_t> make_init_deinit_link_map(llvm::LLVMContext& context, const llvm_type_lookup& type_lookup){
	...
	//	init()
	{
		...
		const auto def = llvm_function_link_entry_t{ "runtime", link_name, type_t::make_undefined(), nullptr, function_type, nullptr, {} };
...
static std::vector<llvm_function_link_entry_t> make_floyd_code_and_corelib_link_map(llvm::LLVMContext& context, const llvm_type_lookup& type_lookup, const std::vector<floyd::function_definition_t>& ast_function_defs){
	...
	//	Make function def for all functions inside the floyd program (floyd source code).
	{
		for(const auto& function_def: ast_function_defs){
			...
			const auto def = llvm_function_link_entry_t{ "program", link_name, function_type, nullptr, (llvm::FunctionType*)function_byvalue_type, nullptr, function_def._named_args };
	...
	for(const auto& e: result0){
		...
		if(it != binds0.end()){
			const auto def2 = llvm_function_link_entry_t{ e.module, e.module_symbol, e.function_type_optional, it->second, e.llvm_function_type, e.llvm_codegen_f, e.arg_names_or_empty };
...
//				throw std::exception();
	...
	for(const auto& e: program_breaks.function_link_map){
		...
		const auto e2 = llvm_function_link_entry_t{ e.module, e.module_symbol, e.function_type_optional, addr, e.llvm_function_type, e.llvm_codegen_f, e.arg_names_or_empty };
