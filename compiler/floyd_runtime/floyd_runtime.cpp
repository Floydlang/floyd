...
value_t unflatten_json_to_specific_type(const type_interner_t& type_interner, const json_t& v, const itype_t& target_type){
	...
	struct visitor_t {
		...
		value_t operator()(const typeid_type_t& e) const{
			const auto typeid_value = itype_from_json(v);
		...
		value_t operator()(const struct_t& e) const{
			if(v.is_object()){
				...
				for(const auto& member: struct_def._members){
					...
					const auto member_value1 = unflatten_json_to_specific_type(type_interner, member_value0, member._type);
					members2.push_back(member_value1);
				}
				const auto result = value_t::make_struct_value(type_interner, target_type, members2);
				return result;
			}
			else{
		...
		value_t operator()(const vector_t& e) const{
			if(v.is_array()){
				const auto target_element_type = target_type.get_vector_element_type(type_interner);
				std::vector<value_t> elements2;
				for(int i = 0 ; i < v.get_array_size() ; i++){
					const auto member_value0 = v.get_array_n(i);
					const auto member_value1 = unflatten_json_to_specific_type(type_interner, member_value0, target_element_type);
					elements2.push_back(member_value1);
				}
				const auto result = value_t::make_vector_value(type_interner, target_element_type, elements2);
				return result;
			}
			else{
		...
		value_t operator()(const dict_t& e) const{
			if(v.is_object()){
				const auto value_type = target_type.get_dict_value_type(type_interner);
				...
				for(const auto& member: source_obj){
					...
					const auto member_value1 = unflatten_json_to_specific_type(type_interner, member_value0, value_type);
					obj2[member_name] = member_value1;
				}
				const auto result = value_t::make_dict_value(type_interner, value_type, obj2);
				return result;
			}
			else{
	...
	return std::visit(visitor_t{ type_interner, target_type, v}, get_itype_variant(type_interner, target_type));
}
