	...
	struct visitor_t {
		...
		itype_t operator()(const typeid_t::struct_t& e) const{
			...
			for(const auto& m: struct_def._members){
				members2.push_back(member_t(intern_anonymous_type(interner, resolver, m._type).second, m._name));
			}
		...
		itype_t operator()(const typeid_t::vector_t& e) const{
			...
			const auto element_type2 = intern_anonymous_type(interner, resolver, e._parts[0]).second;
		...
		itype_t operator()(const typeid_t::function_t& e) const{
			...
			const auto ret2 = intern_anonymous_type(interner, resolver, ret).second;
			std::vector<typeid_t> args2;
			for(const auto& m: args){
				args2.push_back(intern_anonymous_type(interner, resolver, m).second);
			}
...
*/
	...
	const auto result = std::visit(visitor_t{ interner, tag, type, resolver }, type._contents);
...
itype_t new_tagged_type(type_interner_t& interner, const i_resolve_identifer* resolver, const type_tag_t& tag, const typeid_t& type){
	...
	return allocate(interner, resolver, tag, type);
}
...
std::pair<itype_t, typeid_t> intern_anonymous_type(type_interner_t& interner, const i_resolve_identifer* resolver, const typeid_t& type){
	...
	//	New type, store it.
		...
		const auto itype = allocate(interner, resolver, make_empty_type_tag(), type);
		return { itype, lookup_type_from_itype(interner, itype) };
	}
}
...
std::pair<itype_t, typeid_t> intern_anonymous_type(type_interner_t& interner, const i_resolve_identifer* resolver, const ast_type_t& type){
	...
	if(std::holds_alternative<typeid_t>(type._contents)){
		return intern_anonymous_type(interner, resolver, std::get<typeid_t>(type._contents));
	}
	else if(std::holds_alternative<itype_t>(type._contents)){
		const auto& itype = std::get<itype_t>(type._contents);
		const auto t = lookup_type_from_itype(interner, itype);
		return { itype, t };
	}
	else{
		QUARK_ASSERT(false);
...
QUARK_TEST("type_interner_t", "new_tagged_type()", "", ""){
	type_interner_t a;
	const auto r = new_tagged_type(a, nullptr, type_tag_t{{ "a" }}, typeid_t::make_vector(typeid_t::make_identifier("hello")) );
	if(true) trace_type_interner(a);

	const auto find = lookup_tagged_type(a, type_tag_t{{ "a" }});
...
QUARK_TEST("type_interner_t", "new_tagged_type()", "Nested types", "Nested types get their own itypes"){
	type_interner_t a;
	const auto r = new_tagged_type(a, nullptr, type_tag_t{{ "a" }}, typeid_t::make_vector(typeid_t::make_dict(typeid_t::make_double())) );
	if(true) trace_type_interner(a);

	QUARK_UT_VERIFY(lookup_tagged_type(a, type_tag_t{{ "a" }}) == typeid_t::make_vector(typeid_t::make_dict(typeid_t::make_double())) );
...
static long find_tagged_type(const type_interner_t& interner, const type_tag_t& tag){
	QUARK_ASSERT(interner.check_invariant());
...
const typeid_t& lookup_tagged_type(const type_interner_t& interner, const type_tag_t& tag){
	...
	const auto index = find_tagged_type(interner, tag);
	if(index == -1){
		throw std::exception();
	}
	else {
...
//		throw std::exception();
		...
		const auto itype = std::get<itype_t>(type._contents);
		return lookup_type_from_itype(interner, itype);
	}
	else{
		QUARK_ASSERT(false);
		throw std::exception();
...
QUARK_TEST("type_interner_t", "type_interner_t()", "Check that built in types work with lookup_type_from_itype()", ""){
	const type_interner_t a;
	QUARK_UT_VERIFY(lookup_type_from_itype(a, itype_t::make_undefined()) == typeid_t::make_undefined());
	QUARK_UT_VERIFY(lookup_type_from_itype(a, itype_t::make_any()) == typeid_t::make_any());
	QUARK_UT_VERIFY(lookup_type_from_itype(a, itype_t::make_void()) == typeid_t::make_void());

	QUARK_UT_VERIFY(lookup_type_from_itype(a, itype_t::make_bool()) == typeid_t::make_bool());
	QUARK_UT_VERIFY(lookup_type_from_itype(a, itype_t::make_int()) == typeid_t::make_int());
	QUARK_UT_VERIFY(lookup_type_from_itype(a, itype_t::make_double()) == typeid_t::make_double());
	QUARK_UT_VERIFY(lookup_type_from_itype(a, itype_t::make_string()) == typeid_t::make_string());
	QUARK_UT_VERIFY(lookup_type_from_itype(a, itype_t::make_json()) == typeid_t::make_json());

	QUARK_UT_VERIFY(lookup_type_from_itype(a, itype_t::make_typeid()) == typeid_t::make_typeid());
}
