...
std::pair<json_t, seq_t> parse_lhs_atom(const seq_t& p){
	...
	//	(EXPRESSION)xxx"
		...
		return parse_unnamed_struct_type_def(temp, p2);
	}
	...
	*/
		...
		if(a.first._has_keys){
			...
			const auto element_type2 = type_to_json(temp, make_vector(temp, type_t::make_undefined()));
		...
		const auto element_type2 = type_to_json(temp, make_dict(temp, type_t::make_undefined()));
...
QUARK_TEST("parser", "parse_lhs_atom()", "", ""){
	...
	QUARK_VERIFY(a.first == make_parser_node(
		...
		parse_tree_expression_opcode_t::k_value_constructor,
		{
			type_to_json(temp, make_vector(temp, type_t::make_undefined())),
